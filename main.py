...
def stalled_automation(message):
    m = str(open('dedicatedResources/dedicatedResources.txt', 'r').read() +
            ...
            "/plan_support_activities - <b>План действий по поддержке</b>  - Чтобы вы могли связаться со специалистами (например, менеджерами баз данных), когда они вам понадобятся\n\n" +
