...
def start(message):
    ...
    msg = bot.send_message(message.chat.id,
                           text="Добро пожаловать в <b>паттерны для автоматизации тестирования</b>\n\n"
                                ...
                                "/localized_regimes - если проблемы команд существенно отличаются. Это признак того, что каждая команда работает полностью независимо от других, нет обмена экспертизой, методами, стандартами и т. д."
                           , parse_mode='html')
...
def no_previous_test_automation(message):
    msg = bot.send_message(message.chat.id,
                           text=open('patterns/patterns_no_prev_auto.txt', 'r').read() +
                                ...
                                "/automation_roles - <b>Роли автоматизации.</b> Используйте этот шаблон для заполнения ролей, необходимых для разработки тестового программного обеспечения автоматизации. Если возможно, используйте /whole_team_approach - <b>усилия всей команды</b> - и по необходимости /get_training - проводите <b>Обучение для команды</b>\n\n"
...
def localized_regimes(message):
    msg = bot.send_message(message.chat.id,
                           text="<b>Локализованные режимы</b>\n\n"
                                ...
                                "\n/design_for_reuse - <b>Дизайн для переиспользования</b> - роектирование многоразового тестового обеспечения\n\n"
                                ...
                                "/test_automation_owner - <b>Назначенный владелец</b> - назначьте владельца для автоматизации тестирования\n\n"

                                "Другие полезные шаблоны::\n\n"

                                "/get_training - <b>Обучение для команды</b> - планируйте обучение для всех тех, кто участвует в проекте автоматизации тестирования\n\n"
                                "/share_information - <b>Поделиться информацией</b> - запрашивайте и предоставьте информацию менеджерам, разработчикам, другим тестировщикам и клиентам\n\n"
...
def lack_of_support(message):
    msg = bot.send_message(message.chat.id,
                           text="<b>Отсутствие поддержки</b>\n\n"
                                ...
                                "/inadecuate_support - <b>Неадекватная поддержка</b> - если вы столкнулись с отсутствием поддержки сразу в нескольких областях\n\n"
...
def lack_of_resources(message):
    msg = bot.send_message(message.chat.id,
                           text="<b>Нехватка ресурсов</b>\n\n"
                                ...
                                "/unrealistic_expectations - <b>Скользящее расписание</b> - время на автоматизацию не запланировано или его недостаточно\n\n"
